function hout = nd_slice(varargin)
%SLICE  Volumetric slice plot.
%   BTM: Modified to use ndgrid stuff instead of meshgrid   
%
...
if axis_slice,
    h = [];
    [xi,yi,zi] = ndgrid(sx,y(1,:,1),z(1,1,:));
    vi = interpn(x,y,z,v,xi,yi,zi,method);
    for i = 1:length(sx)
        h = [h; surface( ...
    ...
    [xi,yi,zi] = ndgrid(x(:,1,1),sy,z(1,1,:));
    vi = interpn(x,y,z,v,xi,yi,zi,method);
    for i = 1:length(sy)
        h = [h; surface( ...
    ...
    [xi,yi,zi] = ndgrid(x(:,1,1),y(1,:,1),sz);
    vi = interpn(x,y,z,v,xi,yi,zi,method);
    for i = 1:length(sz)
    ...
    vi = interpn(x,y,z,v,sx,sy,sz,method);
